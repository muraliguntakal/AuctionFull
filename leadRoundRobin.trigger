trigger leadRoundRobin on lead (before insert, before update) {
    ...
    Assignment_Group_Queues__c[] agqList = [SELECT Assignment_Group_Name__c, QueueId__c
                                              FROM Assignment_Group_Queues__c 
                                              WHERE QueueId__c in :queueIds.values()
                                              AND Active__c = 'True'];

    system.debug('agqList=> ' +agqList+ ' Size=> '+agqList.size());
    for(Assignment_Group_Queues__c agq : agqList)
    {
        for (Integer i = 0; i < agqList.size() ; i++) {
            asgnGroupQueues.put(agqList[i].QueueId__c, agqList[i]);
        }                                           
    }
    ...
    Assignment_Groups__c[] agsList = [SELECT Group_Name__c, User__c, Last_Assignment__c, Millisecond__c 
    ...
    for(Assignment_Groups__c ags : agsList)
    {
        if (agsList.size()>0) {
            asgnGroups.put(agsList[0].Group_Name__c, agsList);
        }
    }
