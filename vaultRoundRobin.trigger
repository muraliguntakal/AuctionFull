trigger  vaultRoundRobin on Vault__c (before insert, before update) {
    //
    ...
    for (Integer i : queueIds.keySet())
    {
        ...
        if(ags!=null) {
            if (ags.size()>0) {   
                ...
                //Assign User to Vault__c as the new owner
                System.debug('>>>>>Owner changed for Vault__c ' + Trigger.new[i].Id + ' from '+Trigger.new[i].OwnerId+' to '+ ag.User__c);
