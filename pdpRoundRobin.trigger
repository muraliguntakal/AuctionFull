trigger pdpRoundRobin on pdp__c (before insert, before update) {
    //
    ...
    for (Integer i : queueIds.keySet())
    {
        ...
        {   
            ...
            //Assign User to pdp__c as the new owner
            System.debug('>>>>>Owner changed for pdp__c ' + Trigger.new[i].Id + ' from '+Trigger.new[i].OwnerId+' to '+ ag.User__c);
